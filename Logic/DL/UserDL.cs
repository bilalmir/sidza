...
namespace Logic.DL
{
 ...
 /// <summary>
        ...
        public static bool CheckEmailSentToTeacher(string studentemail, string teacheremail)
        {
            ...
            {
                con.Open();
                string qry = "select * from tblcomunication_teach_stu where email='" + studentemail + "' and teacheremail='"+teacheremail+"' and emailflag=1";
                DataSet ds= new DataSet();
        ...
        public static bool ScheduleClass(string teacheremail,string studentemail,string subject,string topic,string grade,string date,string timefrm,string timeto,string link)
        { 
            SqlConnection con = new SqlConnection(DB.GetConnection());
            try
            {
                string query = "insert into tblscheduleclass (teacheremail,studentemail,subject,topic,grade,date,timefrom,timeto,sessionlink)values(@teacheremail,@studentemail,@subject,@topic,@grade,@date,@timefrom,@timeto,@sessionlink)";
                ...
                cmd.Parameters.AddWithValue("@timefrom", timefrm);
                cmd.Parameters.AddWithValue("@timeto", timeto);
                cmd.Parameters.AddWithValue("@sessionlink", link);
                con.Open();
        ...
        public static DataTable getteacheremail(string stuemail)
        {
            ...
            {
                con.Open();
                SqlCommand cmd = new SqlCommand("Select teacheremail from dbo.[tblcomunication_teach_stu] where dbo.[tblcomunication_teach_stu].email ='" + stuemail + "'", con);
                cmd.ExecuteNonQuery();
                SqlDataAdapter da = new SqlDataAdapter(cmd);
                DataSet ds = new DataSet();
                da.Fill(ds);
        ...
        public static DataRow getTeacherProfile(string Teacheremail)
        {
            SqlConnection con = new SqlConnection(DB.GetConnection());
            try
            {
                DataRow dt;
                con.Open();
                SqlCommand cmd = new SqlCommand("select id,name,email,contact,profileimage,teacherSubject,qualification1,qualification2,qualification3,address from [user] where email=@Teacheremail", con);
                SqlDataAdapter da = new SqlDataAdapter(cmd);
                cmd.Parameters.AddWithValue("@Teacheremail", Teacheremail);
                DataSet ds = new DataSet();
                da.Fill(ds);
        ...
        public static DataSet FetchStudentProfile(string email)
        {
            SqlConnection con = new SqlConnection(DB.GetConnection());
            try
            {
                con.Open();
                SqlCommand cmd = new SqlCommand("select U.id,U.name,U.email,C.grade,C.contact from  [user] U inner join tblcomunication_teach_stu C on C.email=U.email where C.emailflag='true' and C.teacheremail=@email", con);
                SqlDataAdapter da = new SqlDataAdapter(cmd);
                cmd.Parameters.AddWithValue("@email",email);
                DataSet ds = new DataSet();
                da.Fill(ds);
        ...
        public static DataSet FetchScheduledClasses(string email)
        {
            SqlConnection con = new SqlConnection(DB.GetConnection());
            try
            {
                con.Open();
                SqlCommand cmd = new SqlCommand("select S.id,U.profileimage,U.name,U.board,S.subject,C.classname,S.date,S.timefrom,S.timeto,S.sessionlink from tblscheduleclass S inner join [user] U on S.studentemail=U.email inner join tblclass C on C.classid=U.classid where S.teacheremail=@email", con);
                SqlDataAdapter da = new SqlDataAdapter(cmd);
                cmd.Parameters.AddWithValue("@email", email);
                DataSet ds = new DataSet();
                da.Fill(ds);
